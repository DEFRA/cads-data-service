...
public class GetHoldingsQueryHandler(HoldingsQueryAdapter adapter)
    : PagedQueryHandler<GetHoldingsQuery, UkvDto>
{
    protected override async Task<(IEnumerable<UkvDto> Items, int TotalCount)> FetchAsync(GetHoldingsQuery query, CancellationToken cancellationToken)
    {
