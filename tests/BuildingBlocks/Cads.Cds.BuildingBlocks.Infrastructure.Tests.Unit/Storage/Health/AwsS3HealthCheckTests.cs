...
namespace Cads.Cds.BuildingBlocks.Infrastructure.Tests.Unit.Storage.Health;
...
public class AwsS3HealthCheckTests
{
    ...
    public AwsS3HealthCheckTests()
    {
        ...
        _s3ClientFactoryMock.Setup(x => x.GetRegisteredClientNames())
            .Returns([typeof(TestStorageClientA).Name]);
    ...
    public async Task GivenS3BucketsAreNotFound_WhenRequestingHealthCheck_ShouldFail()
    {
        ...
        result.Description.Should().Be($"Some S3 buckets failed: {DefaultBucketName}");
    }
    ...
    [Fact]
    public async Task HealthCheck_Returns_Unhealthy_When_Bucket_NotFound()
    {
        ...
        result.Status.Should().Be(HealthStatus.Unhealthy);
        result.Description.Should().Be("Some S3 buckets failed: test-bucket");
        result.Data.Should().ContainKey("TestStorageClientA");
    }
