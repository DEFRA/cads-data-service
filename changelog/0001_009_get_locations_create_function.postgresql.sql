...
select
    ...
    lif.lif_description,
    l.imported_date
from _ct_locations l
left join _ct_location_types lty
    on lty.lty_id = l.loc_lty_id
left join _ct_sublocation_types slt
    on slt.slt_id = l.loc_slt_id
left join _ct_counties cty
    on cty.cty_id = l.loc_cty_id
left join _ct_location_id_formats lif
    on lif.lif_id = lty.lty_lif_id
left join _ct_addresses adr
    on adr.adr_loc_id = l.loc_id
left join _ct_location_identifiers lid
    on lid.lid_loc_id = l.loc_id
left join _ct_location_relationships llr_parent
    on llr_parent.llr_loc_id_parent = l.loc_id
left join _ct_location_relationships llr_child
    on llr_child.llr_loc_id_child = l.loc_id
left join _ct_location_rel_types lrt_parent
    on lrt_parent.lrt_id = llr_parent.llr_lrt_id
left join _ct_param_value pvl_vet
    on pvl_vet.pvl_param = 'CP.VTAREA'
   and pvl_vet.pvl_param_value = cty.cty_vet_area
left join _ct_param_value_group pvg_passport
