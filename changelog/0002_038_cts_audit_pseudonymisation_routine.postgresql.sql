...
-- changeset dev_db_build:0002-038-cts-audit-pseudonymisation splitStatements:false
CREATE OR REPLACE PROCEDURE cads.cts_audit_pseudonymise_data()
LANGUAGE plpgsql
AS $procedure$
DECLARE
    v_table_name text;
...
BEGIN
v_table_name := 'cts_audit.ct_parties';
UPDATE cts_audit.ct_parties
...
v_table_name := 'cts_audit.ct_locations';
UPDATE cts_audit.ct_locations
...
v_table_name := 'cts_audit.ct_addresses';
UPDATE cts_audit.ct_addresses
...
v_table_name := 'cts_audit.ct_label_requests';
UPDATE cts_audit.ct_label_requests
...
v_table_name := 'cts_audit.ct_web_users';
UPDATE cts_audit.ct_web_users
...
v_table_name := 'cts_audit.ct_comms_addresses';
UPDATE cts_audit.ct_comms_addresses
SET coa_email_address = 'coa_email_' || coa_id::text || '@defra.gov.uk'
WHERE coa_id IS NOT NULL;
GET DIAGNOSTICS v_rows_updated = ROW_COUNT;
RAISE NOTICE '%: % records updated', v_table_name, v_rows_updated;

v_table_name := 'cts_audit.ct_cts_users';
UPDATE cts_audit.ct_cts_users
...
v_table_name := 'cts_audit.ct_email_log';
UPDATE cts_audit.ct_email_log
...
v_table_name := 'cts_audit.ct_location_party_rels';
UPDATE cts_audit.ct_location_party_rels
SET lpr_comments = 'LPR-COMMENT-' || lpr_id::text
WHERE lpr_id IS NOT NULL;
GET DIAGNOSTICS v_rows_updated = ROW_COUNT;
RAISE NOTICE '%: % records updated', v_table_name, v_rows_updated;

v_table_name := 'cts_audit.ct_location_relationships';
UPDATE cts_audit.ct_location_relationships
SET llr_comments = 'LLR-COMMENT-' || llr_id::text
WHERE llr_id IS NOT NULL;
GET DIAGNOSTICS v_rows_updated = ROW_COUNT;
RAISE NOTICE '%: % records updated', v_table_name, v_rows_updated;

v_table_name := 'cts_audit.ct_condition_markers';
UPDATE cts_audit.ct_condition_markers
SET com_comments = 'COM-COMMENT-' || com_id::text
WHERE com_id IS NOT NULL;
GET DIAGNOSTICS v_rows_updated = ROW_COUNT;
RAISE NOTICE '%: % records updated', v_table_name, v_rows_updated;

v_table_name := 'cts_audit.ct_susp_condition_markers';
UPDATE cts_audit.ct_susp_condition_markers
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'cts_audit pseudonymisation failed while updating %: %', v_table_name, SQLERRM
